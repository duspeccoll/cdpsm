...
Nokogiri::XML.parse(File.read(input)).xpath("/metadata/oai_dc:dc").each_with_index do |node, i|
  ...
  ids = []
  node.xpath("dc:identifier").each_with_index do |identifier|
    case
    ...
    when identifier.text.start_with?('http://swcenter.fortlewis.edu/images/M194/')
      ids.push(identifier.text.gsub("http://swcenter.fortlewis.edu/images/M194/",'').gsub(/\..+$/,''))
    end
  end
  case
  when ids.length == 0
