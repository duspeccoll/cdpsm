...
Nokogiri::XML.parse(File.read(input)).xpath("/metadata/oai_dc:dc").each do |node|
  ...
  # customization for CoAsAHS: handle identifier case for images with cdpheritage.org URIs
  ids = []
  node.xpath("dc:identifier").each do |identifier|

    case
    when identifier.text.start_with?('http://dcbuilder.bcr.org')
      ids.push(identifier.text.gsub("http://dcbuilder.bcr.org/streaming/index.cfm?filename=",'').gsub(/\..+$/,''))
    when identifier.text.start_with?('http://www.cdpheritage.org')
      ids.push(identifier.text.gsub(/http:\/\/www.cdpheritage.org\/soundModel\/aspen\/\w+\//,'').gsub(/\.\w+$/,'').gsub(/\./,'_'))
    end
  end
  if ids.length == 1
    id = ids[0]
