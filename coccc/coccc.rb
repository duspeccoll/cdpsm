...
Nokogiri::XML.parse(File.read(input)).xpath("/metadata/oai_dc:dc").each do |node|
  ...
  # CoCCC modification: sometimes the '=' is missing from the URL so I accounted for that
  ids = []
  ...
  builder = Nokogiri::XML::Builder.new(:encoding => 'UTF-8') do |xml|
    xml.mods({
      ...
      node.xpath("dc:description").each do |description|
        ...
        when /^Computer hardware:/, /^Sound quality note:/, /^Playback:/
      ...
      node.xpath("dc:source").each do |source|
        xml.note(:type => 'original location') {
          xml.text source.text.strip
        }
      end
